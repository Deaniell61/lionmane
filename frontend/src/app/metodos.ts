...
export class Sesion implements OnInit {
  ...
  public validarSesion(): boolean {
    ...
    if (tempData && !this._perfil) {
      perfil = tempData ? JSON.parse(this.desencriptar(tempData)) : null;
  ...
  public actualizaPerfil(perf?: Perfil): void {
    if (perf) {
      ...
      let perfil: Perfil;
      if (tempData) {
        perfil = this._perfil ? this._perfil : JSON.parse(this.desencriptar(tempData));
        if (perfil) {
      ...
      if (this.currentPerfil && this.currentPerfil !== 'dW5kZWZpbmVk') {
        const per = this.currentPerfil ? JSON.parse(this.desencriptar(this.currentPerfil)) : null;
        if (per) {
          perfil = per;
        }
      }
